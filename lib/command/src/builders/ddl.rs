...
//!
//! Higher-level builders return [`crate::program::Program`] directly. They take
//! ownership of name / namespace / field metadata and lower it onto the
//! [`Target`](crate::target::Target) + [`SchemaRef`] shape.
...
/// [`SchemaCatalog`](dol_schema::SchemaCatalog). When the catalog is unknown,
...
pub fn define_lookup(
    ...
    use crate::operation::{LookupMethod, StructuralVerb};
    use smallvec::SmallVec;

    let mut interner = Interner::new();
    let target = target_from_parts(&mut interner, TargetKind::Relation, table, namespace);
    let name_sym = super::target::intern_symbol(&mut interner, name);
    let cols: SmallVec<[crate::target::Symbol; 2]> = columns
        .iter()
        .map(|c| super::target::intern_symbol(&mut interner, c))
        .collect();
...
pub fn drop_field(table: &str, namespace: Option<&str>, field: &str) -> Program {
    use crate::operation::StructuralVerb;

    let mut interner = Interner::new();
    let target = target_from_parts(&mut interner, TargetKind::Relation, table, namespace);
    let field_sym = super::target::intern_symbol(&mut interner, field);
...
pub fn rename_field(table: &str, namespace: Option<&str>, from: &str, to: &str) -> Program {
    ...
    let mut interner = Interner::new();
    let target = target_from_parts(&mut interner, TargetKind::Relation, table, namespace);
    let from_sym = super::target::intern_symbol(&mut interner, from);
    let to_sym = super::target::intern_symbol(&mut interner, to);
    let op: Operation = FieldOp {
...
pub fn define_index(
    ...
    namespace: Option<&str>,
    name: &str,
    keys: smallvec::SmallVec<[crate::operation::IndexKey; 2]>,
    unique: bool,
    method: crate::operation::IndexMethod,
    if_not_exists: bool,
) -> Program {
    use crate::operation::StructuralVerb;
