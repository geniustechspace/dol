...
//!
//! The flat `dol_ir::prelude::*` import keeps the existing one-line
//! "give me everything" ergonomic. The submodules
//! ([`dml`], [`ddl`], [`dql`], [`acl`], [`tx`]) mirror the
//! `dol_ir::operation` namespace split, so consumers can opt into a scoped
//! import — `use dol_ir::prelude::dml::*;` — when they only need one
