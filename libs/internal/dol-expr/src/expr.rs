...
mod field_tests {
    ...
    fn bare_field_roundtrips() {
        ...
        let fid    = arena.alloc_field(FieldNode { namespace: None, column: col_id, steps: smallvec![] });
        let nid    = arena.alloc(ExprNode::Field(fid));

        let ExprNode::Field(got_fid) = arena.get(nid) else { panic!("expected Field") };
    ...
    fn namespace_node_roundtrips() {
        ...
        let ExprNode::Namespace(got_id) = arena.get(nid) else { panic!("expected Namespace") };
    ...
    fn session_field_helper() {
        ...
        let ExprNode::Field(fid) = sess.arena.get(nid) else { panic!("expected Field") };
    ...
    fn session_namespace_helper() {
        ...
        let ExprNode::Namespace(id) = sess.arena.get(nid) else { panic!("expected Namespace") };
    ...
    fn session_deep_rest_path() {
        ...
        let nid = sess.qualified_field(
            "response",
            "data",
            smallvec![FieldStep::Key(items_id), FieldStep::Index(0), FieldStep::Key(name_id)],
        );
