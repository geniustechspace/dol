...
impl<'a> Literal<'a> {
    ...
    pub fn float32(v: f32) -> Result<Self, crate::TypeError> {
        if !v.is_finite() { return Err(crate::TypeError::NonFiniteFloat); }
        Ok(Self::Float32(v))
    }

    pub fn float64(v: f64) -> Result<Self, crate::TypeError> {
        if !v.is_finite() { return Err(crate::TypeError::NonFiniteFloat); }
        Ok(Self::Float64(v))
    }
    ...
    pub fn decimal(unscaled: i128, scale: u32) -> Result<Self, crate::TypeError> {
    ...
    pub fn date(year: i32, month: u8, day: u8) -> Result<Self, crate::TypeError> {
        match Date::try_new(year, month, day) {
            Ok(d) => Ok(Self::Date(d)), Err(e) => Err(e),
        }
    }

    pub fn time(h: u8, m: u8, s: u8, ns: u32) -> Result<Self, crate::TypeError> {
    ...
    pub fn point(x: f64, y: f64) -> Result<Self, crate::TypeError> {
...
mod tests {
    ...
    fn bitstring_length_mismatch_rejected() {
        ...
        assert!(matches!(err, crate::TypeError::BitLengthMismatch { declared: 9, byte_count: 1 }));
    }
