...
impl DataType {
    ...
    pub const fn is_textual(&self) -> bool {
        matches!(self, Self::Char(_) | Self::Varchar(_) | Self::Text | Self::Json | Self::Xml)
    }
    ...
    pub const fn is_geometric(&self) -> bool {
        matches!(self, Self::Point | Self::Line | Self::LineSegment
                     | Self::Rect | Self::Circle | Self::Path | Self::Polygon)
    }

    pub const fn is_network(&self) -> bool {
        matches!(self, Self::Inet | Self::Cidr { .. } | Self::MacAddr | Self::MacAddr8)
    }
        ...
        match self {
            ...
            Self::Char(len) => {
                let V::String(s) = value else {
                    return Err(TypeError::KindMismatch { expected: self.type_name(), got: value.type_name() });
                };
                let actual = s.chars().count();
                if actual != *len as usize {
                    // CHAR requires exact length; backends pad, but we validate the max.
                    if actual > *len as usize {
                        return Err(TypeError::StringTooLong { max: *len, got: actual });
                    }
                }
                Ok(())
            }
            Self::Varchar(max) => {
                ...
                if let Some(max_len) = max {
                    ...
                    if actual > *max_len as usize {
                        return Err(TypeError::StringTooLong { max: *max_len, got: actual });
                    }
                }
                Ok(())
            }
            ...
            Self::Binary(len) => {
                ...
                if b.len() != *len as usize {
                    return Err(TypeError::BytesTooLong { max: *len, got: b.len() });
                }
                Ok(())
            }
            Self::Varbinary(max) => {
                ...
                if let Some(max_len) = max {
                    if b.len() > *max_len as usize {
                        return Err(TypeError::BytesTooLong { max: *max_len, got: b.len() });
                    }
                }
                Ok(())
            }
            ...
            Self::Bit(len) => {
                ...
                if bs.len != *len {
                    return Err(TypeError::BitsLengthMismatch { expected: *len, got: bs.len });
                }
                Ok(())
            }
            Self::Varbit(max) => {
                ...
                if let Some(max_bits) = max {
                    if bs.len > *max_bits {
                        return Err(TypeError::BitsTooLong { max: *max_bits, got: bs.len });
                    }
                }
                Ok(())
            }
            ...
            Self::Array(element_type) => {
                ...
                for (i, item) in items.iter().enumerate() {
                    element_type.accepts(item).map_err(|e| {
                        TypeError::ElementInvalid { index: i, source: Box::new(e) }
                    })?;
                }
                Ok(())
            }
            ...
            Self::Map { value: val_type } => {
                ...
                for (k, v) in entries.iter() {
                    val_type.accepts(v).map_err(|e| {
                        TypeError::MapValueInvalid { key: k.clone(), source: Box::new(e) }
                    })?;
                }
                Ok(())
            }
            ...
            Self::Tuple(field_types) => {
                ...
                if items.len() != field_types.len() {
                    return Err(TypeError::TupleLengthMismatch {
                        expected: field_types.len(), got: items.len()
                    });
                }
                for (i, (ft, v)) in field_types.iter().zip(items.iter()).enumerate() {
                    ft.accepts(v).map_err(|e| {
                        TypeError::ElementInvalid { index: i, source: Box::new(e) }
                    })?;
                }
                Ok(())
            ...
            Self::Struct(fields) => {
                ...
                for field in fields {
                    match entries.iter().find(|(k, _)| k.as_ref() == field.name.as_ref()) {
                        None => {
                            if !field.nullable {
            ...
            Self::Enum(variants) => {
                ...
                if !variants.iter().any(|v| v.as_ref() == variant.as_ref()) {
                    return Err(TypeError::EnumVariantUnknown { variant: variant.clone() });
                }
                Ok(())
            }
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::Decimal { precision: Some(p), scale: Some(s) } =>
                write!(f, "NUMERIC({p},{s})"),
            Self::Decimal { precision: Some(p), scale: None } =>
                write!(f, "NUMERIC({p})"),
            Self::Decimal { .. }  => write!(f, "NUMERIC"),
            Self::Time { precision: 6 } | Self::Time { .. } => {
                if let Self::Time { precision } = self { write!(f, "TIME({precision})") }
                else { unreachable!() }
            }
...
mod tests {
    ...
    fn decimal_precision_enforced() {
        let dt = DataType::Decimal { precision: Some(4), scale: Some(2) };
    ...
    fn decimal_scale_enforced() {
        let dt = DataType::Decimal { precision: None, scale: Some(2) };
    ...
    fn enum_rejects_unknown_variant() {
        ...
        let err = dt.accepts(&v).unwrap_err();
        assert!(matches!(err, TypeError::EnumVariantUnknown { ref variant } if variant.as_ref() == "deleted"));
    }
    ...
    fn tuple_length_mismatch() {
        ...
        assert!(matches!(
            ...
            TypeError::TupleLengthMismatch { expected: 2, got: 1 }
        ));
    }
    ...
    fn display_for_datatypes() {
        ...
        assert_eq!(DataType::Array(Box::new(DataType::Int32)).to_string(), "INT32[]");
        assert_eq!(DataType::Decimal { precision: Some(10), scale: Some(2) }.to_string(), "NUMERIC(10,2)");
    }
}
