...
#![deny(unsafe_code)]
use dol_ir::{BackendError, Statement};
...
impl ObjectStorageBackend {
    pub fn render(&self, stmt: &Statement) -> Result<StorageOutput, BackendError> {
        match stmt {
            ...
            _ => Err(BackendError::Unsupported("ObjectStorageBackend only supports storage operations".into())),
        }
    }
}
#[cfg(test)]
