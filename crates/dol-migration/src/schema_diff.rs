...
//!
//! ```rust
//! use dol_model::{Model, Field, FieldType};
...
use dol_ir::definition::{FieldDef, OwnedForeignKeyRef};
use dol_ir::AlterAction;
...
pub fn field_to_field_def(field: &Field) -> FieldDef {
    ...
    if let Some((kind, expr)) = &field.generated {
        def = match kind {
            dol_model::constraint::GeneratedKind::Stored => def.generated_stored(expr),
            dol_model::constraint::GeneratedKind::Virtual => def.generated_virtual(expr),
        };
    }
    def
}
