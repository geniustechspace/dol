...
//!
//! - **`dol-core`** — Expression engine (`Expr`, operators, functions)
//! - **`dol-model`** — Schema language (`Model`, `Field`, `FieldType`)
//! - **`dol-ir`** — Intermediate representation (backend-agnostic AST)
//! - **`dol-builder`** — Composable builder API
...
pub mod backend {
    /// Backend trait and shared output types (from dol-ir).
    pub use dol_ir::{
...
pub use dol_model::constraint::{FkAction, ForeignKeyRef, GeneratedKind, ModelConstraint};
pub use dol_model::{Field, FieldType, Model};
...
pub use dol_builder::{ModelBuilderExt, ModelDefineExt};
