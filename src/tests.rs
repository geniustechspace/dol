...
use crate::expr::{case, col, field, lit, param, raw_expr, Direction, Expr, NullsPosition};
...
fn select_exists_subquery() {
    let sql = USERS
        ...
        .to_sql(Some(&pg()));
    assert!(sql.contains("WHERE EXISTS (SELECT 1 FROM sessions WHERE sessions.user_id = users.id)"));
}
